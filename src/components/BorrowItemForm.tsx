...
import { useBorrowItem } from "../hooks/item/useBorrowItem";
...
export const BorrowItemForm = ({
  ...
  const [scannedItemDetails, setScannedItemDetails] =
    useState<TItemList | null>(null);
  const [isScannedItem, setIsScannedItem] = useState<boolean>(false);
  const [itemIdError, setItemIdError] = useState<string>("");
  const [borrowerFirstNameError, setBorrowerFirstNameError] =
    useState<string>("");
  const [borrowerLastNameError, setBorrowerLastNameError] =
    useState<string>("");
  const [borrowerTeacherFirstNameError, setBorrowerTeacherFirstNameError] =
    useState<string>("");
  const [borrowerTeacherLastNameError, setBorrowerTeacherLastNameError] =
    useState<string>("");
  const [borrowerStudentIdNumberError, setBorrowerStudentIdNumberError] =
    useState<string>("");
  const [borrowerRoleError, setBorrowerRoleError] = useState<string>("");
  const [roomError, setRoomError] = useState<string>("");
  const [subjectTimeScheduleError, setSubjectTimeScheduleError] =
    useState<string>("");
  ...
  const { data } = useQuery(useAllItems());
  ...
  const handleScanSubmit = async () => {
    ...
    try {
      ...
      if (foundItem.status !== "Available") {
        setScanError(
          "This item is currently unavailable. Please select a different item.",
        );
        return;
      }
  ...
  const handleReservationSubmit = async () => {
    ...
    try {
      ...
      await borrowItemMutation.mutateAsync({
        ...
        teacherLastName:
          formData.borrowerRole === "Teacher"
            ? "yes"
            : formData.teacherLastName || null,
        room: formData.room,
  ...
  return (
    <>
          ...
          >
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">
                Scan the item barcode or manually enter it to select an item for
                borrowing.
              </p>
              <input
                type="text"
      ...
      <Activity mode={showReservationModal ? "visible" : "hidden"}>
        <ReservationConfirmModal
          getreservationDateError={reservationDateError}
          getreservationTimeError={reservationTimeError}
          setShowReservationModal={() => setShowReservationModal(true)}
          handleReservationSubmit={handleReservationSubmit}
          borrowItemMutation={borrowItemMutation.isPending}
        />
      </Activity>
              ...
              <div className="p-4 md:p-6">
                ...
                <div className="text-center mb-4">
                  <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-1">
                    {scannedItemDetails.itemName}
                  </h3>
                  <p className="text-gray-600 font-semibold">
                    {scannedItemDetails.category}
                  </p>
                  {scannedItemDetails.barcode && (
                    <p className="text-gray-500 text-sm mt-1">
                      Barcode: {scannedItemDetails.barcode}
                    </p>
                  )}
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                  <div className="flex justify-center">
                    {scannedItemDetails.image ? (
                      <img
                        src={
                          typeof scannedItemDetails.image === "string"
                            ? scannedItemDetails.image
                            : box
                        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-gray-600 ml-11 text-sm">
                      {scannedItemDetails.serialNumber}
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-gray-600 ml-11 text-sm">
                      {scannedItemDetails.itemModel || "N/A"}
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-gray-600 ml-11 text-sm">
                      {FormattedDateTime(scannedItemDetails.createdAt)}
                    </p>
                  </div>
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            <p className="text-sm text-blue-600 font-medium mb-6">
              ...
              Use the "Scan Item" button to select an item by scanning its
              barcode.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className="md:col-span-2">
              ...
              >
                ...
                {isScannedItem && (
                  <span className="ml-2 text-xs text-blue-600">(Scanned)</span>
                )}
                <span className="ml-2 text-xs text-gray-500">
                  (Use Scan Item button)
                </span>
              </label>
              <input
