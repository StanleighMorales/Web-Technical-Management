...
const BorrowItemForm = ({ prefilledItemId, prefilledItemName }: { prefilledItemId?: string; prefilledItemName?: string; onLentItemScanned?: (lentItem: any) => void }) => {
    ...
    const handleReservationSubmit = async () => {
        ...
        if (!reservationDate) {
            setReservationDateError("Please select a date");
            hasError = true;
        }

        if (!reservationTime) {
            setReservationTimeError("Please select a time");
            hasError = true;
        }

        // Validate time is between 7:30 AM and 8:30 PM
        if (reservationTime) {
            const [hours, minutes] = reservationTime.split(':').map(Number);
            const timeInMinutes = hours * 60 + minutes;
            const minTime = 7 * 60 + 30; // 7:30 AM
            const maxTime = 20 * 60 + 30; // 8:30 PM

            if (timeInMinutes < minTime || timeInMinutes > maxTime) {
                setReservationTimeError("Time must be between 7:30 AM and 8:30 PM");
                hasError = true;
            }
        }
        ...
        try {
            // Combine date and time into ISO format for reservedFor
    ...
    const handleSubmit = async (e: React.FormEvent, status: 'Borrowed' | 'Reserved' = 'Borrowed') => {
        e.preventDefault();
        let hasError = false;
        ...
        // If status is Reserved, show the date/time modal
        if (status === 'Reserved') {
            setShowReservationModal(true);
            return;
        }

        // For 'Borrowed' status, submit directly
        try {
            await borrowItemMutation.mutateAsync({
                itemId: formData.itemId,
                borrowerFirstName: formData.borrowerFirstName,
    ...
    return (
        <>
            {showAlert && <SuccessAlert message={successMessage || "Request Submitted Successfully"} />}
            ...
            {/* Reservation Date/Time Modal */}
            {showReservationModal && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowReservationModal(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
                        {/* Modal Header */}
                        <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center rounded-t-2xl">
                            <h2 className="text-xl font-bold text-gray-900">Select Reservation Date & Time</h2>
                            <button
                        ...
                        <div className="p-6 space-y-4">
                            <p className="text-sm text-gray-600 mb-4">
                                Choose when you'd like to pick up the reserved item. Sundays are not available.
                            </p>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (selectedDate.getDay() === 0) {
                                            setReservationDateError("Sundays are not available for reservations");
                                            setReservationDate("");
                                        } else {
                                ...
                                {reservationDateError && (
                                    <p className="text-red-500 text-sm mt-2">{reservationDateError}</p>
                                )}
                            </div>
                            ...
                            <div>
                                <label htmlFor="reservationTime" className="block text-sm font-medium text-gray-700 mb-2">
                                    ...
                                    <span className="text-xs text-gray-500 ml-2">(7:30 AM - 8:30 PM)</span>
                                </label>
                                <input
                                    type="time"
                                    id="reservationTime"
                                ...
                                {reservationTimeError && (
                                    <p className="text-red-500 text-sm mt-2">{reservationTimeError}</p>
                                )}
                            </div>
                            ...
                            {reservationDate && reservationTime && (
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                    <p className="text-sm text-blue-800">
                                        <strong>Reservation:</strong> {new Date(reservationDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} at {new Date(`2000-01-01T${reservationTime}`).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="flex gap-3 mt-6">
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-4 py-2 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 transition-colors ${borrowItemMutation.isPending ? 'opacity-50 cursor-not-allowed' : ''
                                        }`}
                                >
                ...
                <form onSubmit={handleSubmit} className="space-y-2">
                    ...
                    <div className="flex justify-center gap-3 pt-2">
                        <button
                            ...
                            disabled={borrowItemMutation.isPending}
                            className={`flex items-center gap-2 px-6 py-2 bg-purple-600 text-white font-medium rounded-lg shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition ${borrowItemMutation.isPending ? 'opacity-50 cursor-not-allowed' : ''
                                }`}
                        ...
                        <button
                            type="submit"
                            disabled={borrowItemMutation.isPending}
                            className={`flex items-center gap-2 px-6 py-2 bg-blue-600 text-white font-medium rounded-lg shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ${borrowItemMutation.isPending ? 'opacity-50 cursor-not-allowed' : ''
                                }`}
