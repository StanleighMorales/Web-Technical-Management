    ...
    const handleSubmit = async (e: React.FormEvent, status: 'Borrowed' | 'Reserved' = 'Borrowed') => {
        ...
        try {
            await borrowItemMutation.mutateAsync({
                ...
                status: status, // Pass the status (Borrowed or Reserved)
            });

            setSuccessMessage(status === 'Reserved' ? 'Reservation Submitted Successfully' : 'Borrow Request Submitted Successfully');
            setShowAlert(true);
            setTimeout(() => {
                setShowAlert(false);
                setSuccessMessage("");
